...
function createCustomNavbar() {
    ...
    const h1Elements = document.querySelectorAll('.bd-main h1, .bd-content h1, main h1, .document h1');
...
function createNavbarFromElements(h1Elements) {
    ...
    h1Elements.forEach((h1, index) => {
        ...
        if (!h1.id) {
            ...
            h1.id = text.replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
        }
        ...
        let cleanText = h1.textContent.trim().replace(/^#+\s*|\s*#+$/g, '').trim();
