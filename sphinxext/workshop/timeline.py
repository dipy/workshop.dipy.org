...
class TimelineItemDirective(SphinxDirective):
    ...
    def run(self):
        ...
        if not hasattr(env, 'current_timeline_items') or env.current_timeline_items is None:
            logger.warning("TimelineItemDirective used outside a TimelineDirective context.", location=self.get_location())
            return []
